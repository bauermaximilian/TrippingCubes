...
#BUG: If a vertex has muliple UVs, the vertex UVs of the mesh output are incorrect.
#     Temporary fix: Go into Edit mode, then Mesh>Split>"Faces by Edges".
...
if len(bpy.context.selected_objects) == 0:
    ...
    #At the same time, combine the referenced vertices with their color and UV data 
    #and put them into the vertices array.
    ...
    #Iterate through the created vertex array and create a valid string/XML representation of each.
    ...
    #Iterate through the created face array and create a valid string/XML representation of each.
    blockFaces = "<faces>\n"
    for f in faces:
        blockFaces += "\t%s %s %s\n" % (f[0], f[2], f[1])
    blockFaces += "</faces>"    

    #Combine the previously created string representations of vertices and faces and put them into the clipboard.
    #Confirm the successful end of the operation to the user.
    bpy.context.window_manager.clipboard = blockVertices + "\n" + blockFaces
    show_message("The block mesh vertex and face definitions were copied to the clipboard!",
